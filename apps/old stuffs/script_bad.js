...
document.addEventListener("DOMContentLoaded", function() {
    async function pollForFile() {
        try {
            ...
            if (response.ok) {
                console.log("Getting the data content!!");
                const data = await response.text(); // Awaiting the text response
                console.log("Print the data content!!");
                console.log(data);
                
                if (data) { // Check if data is non-empty
                    document.getElementById('audioSummary').value = data; // Update the textbox value
                    clearInterval(intervalId); // Stop polling when the file is received
                }
            } else {
                console.error('Failed to fetch file:', response.status);
            }
